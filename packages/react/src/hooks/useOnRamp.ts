/**
 * useOnRamp — hook for on-ramp and off-ramp operations
 */
...
export function useOnRamp(): UseOnRampReturn {
  ...
  // Initialize engine lazily from provider config
  const getEngine = useCallback(() => {
    ...
    if (auth.session?.token) {
      engineRef.current.setAuthToken(auth.session.token);
    }
  ...
  const buyWithQuote = useCallback(async (params: {
    quoteId: string;
    paymentDetails?: BankAccount | MobileMoneyAccount;
  }) => {
  ...
  const getOffRampQuote = useCallback(async (params: {
    cryptoAmount: string;
    cryptoAsset: string;
    chain: 'evm' | 'solana';
    fiatCurrency: FiatCurrency;
  ...
  const sellWithQuote = useCallback(async (params: {
    quoteId: string;
    payoutDetails: BankAccount | MobileMoneyAccount;
  }) => {
    if (!wallet.activeAccount) throw new Error('No active wallet account');
