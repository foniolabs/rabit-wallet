/**
 * usePortfolioTotal — sum your token balances into a single USD figure.
 *
 * Uses CoinGecko's `simple/price` endpoint with 60s caching. Only prices
 * tokens we have a CoinGecko ID for; unknown tokens contribute zero.
 */
...
const PRICE_CACHE: Record<string, PriceCacheEntry> = {};
...
async function getPrices(symbols: string[]): Promise<Record<string, number>> {
  ...
  } catch {/* network — leave caller with whatever cached prices we have */}

  return out;
}

export interface PortfolioTotalResult {
  /** Total USD across all priced tokens. null while loading. */
...
export function usePortfolioTotal(): PortfolioTotalResult {
  ...
  // Fetch prices whenever balance symbols change.
  const balanceKey = useMemo(
    () => balances.map((b) => b.symbol).sort().join(','),
    [balances]
  );
  ...
  const refresh = useMemo(
    () => async () => {
      ...
      setIsLoading(true);
      try {
        const next = await getPrices(balances.map((b) => b.symbol));
        setPrices(next);
      } finally {
        setIsLoading(false);
      }
