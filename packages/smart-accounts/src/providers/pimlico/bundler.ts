/**
 * src/providers/pimlico/bundler.ts
...
export class PimlicoBundlerClient implements IBundlerClient {
  ...
  constructor(config: PimlicoConfig) {
    this.config = config
    this.rpcUrl = config.bundlerUrl || `https://api.pimlico.io/v1/${this.getChainName(config.chainId)}/rpc?apikey=${config.apiKey}`
  }
  ...
  async getGasPrice(): Promise<{ slow: bigint; standard: bigint; fast: bigint }> {
    try {
      const result = await this.rpcCall('pimlico_getGasPrice', [])
      return {
        slow: BigInt(result.slow),
        standard: BigInt(result.standard),
        fast: BigInt(result.fast)
      }
    } catch (error) {
